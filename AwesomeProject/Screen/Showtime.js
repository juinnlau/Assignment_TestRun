      ...
      {imageURL ? (
        <View style={styles.contentContainer}>
          ...
          <View style={styles.dateButtonContainer}>
            <DateButton date={todayDate} day={todayDay} onPress={handleDateButtonPress} selected={selectedDate === todayDate} />
            <DateButton date={nextDate1} day={nextDay1} onPress={handleDateButtonPress} selected={selectedDate === nextDate1} />
            <DateButton date={nextDate2} day={nextDay2} onPress={handleDateButtonPress} selected={selectedDate === nextDate2} />
          </View>

          <Text style={styles.availableTimeslotText}>Available Timeslot</Text>

          {selectedDate && (
            <ShowtimesList showtimes={showtimes[selectedDate]} onPress={handleShowtimePress} selectedShowtime={selectedShowtime} />
          )}

          <TouchableOpacity style={styles.selectSeatButton} onPress={handleSelectSeat}>
...
const DateButton = ({ date, day, onPress, selected }) => {
  return (
    <TouchableOpacity style={[styles.dateButton, selected && styles.selectedDateButton]} onPress={() => onPress(date)}>
      <Text style={[styles.dateText, selected && styles.selectedDateText]}>{date}</Text>
        ...
        <View key={columnIndex} style={styles.showtimeColumn}>
          {columns.map((showtimeColumn, rowIndex) => (
            <TouchableOpacity
              ...
              style={[
                styles.showtimeItem,
                selectedShowtime === showtimeColumn[columnIndex] && styles.selectedShowtimeItem,
              ]}
              onPress={() => onPress(showtimeColumn[columnIndex])}
            >
              <Text style={[
                styles.showtimeText,
                selectedShowtime === showtimeColumn[columnIndex] && styles.selectedShowtimeText,
              ]}>
